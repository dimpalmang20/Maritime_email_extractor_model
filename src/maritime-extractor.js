...
exports.toEnterpriseEntry = toEnterpriseEntry;
exports.validateEnterpriseEntry = validateEnterpriseEntry;
exports.extractMaritimeEmail = extractMaritimeEmail;
exports.extractToEnterpriseJSON = extractToEnterpriseJSON;
// Maritime Email Extraction Engine — Enterprise Grade v2.0
// Rule-Based + Heuristic NLP | Dynamic Schema | Multi-Order | High Accuracy
const mlConnector_js_1 = require("./ml/mlConnector.js");
...
function isValidPort(text) {
    ...
    if (t.length <= 2)
        return false;
    if (REGION_MAP[t] !== undefined)
        return false;
    if (t.length <= 3 && !PORT_ABBREVS[t])
        return false;
    if (/^\d/.test(t))
        return false;
    if (/^\d+$/.test(t))
    ...
    if (/^(?:MOLOO|MOLCO|FIOST|FIOS|LINER|DLOSP|CHOPT|APS|AFS|TIP|DIRECT)$/i.test(t))
        return false;
    return true;
}
function isValidCargo(name) {
    if (!name)
        return false;
    const t = name.trim().toUpperCase();
    if (t.length < 3)
        return false;
    if (CARGO_BLACKLIST.has(t))
        return false;
    ...
    if (words.every(w => SHIP_PART_WORDS.has(w)))
        return false;
    return true;
}
...
function normalizeDwtNumber(raw) {
    if (!raw)
        return "";
...
function removeMobileFooters(text) {
    return text
...
function compactForFallback(text) {
...
function inferYear(monthNum) {
    ...
    if (targetMonth < curMonth - 1)
        return (curYear + 1).toString();
    return curYear.toString();
}
function parseLaycan(text) {
...
function parseDwt(text) {
    ...
    if (labelPrefixRange) {
        ...
        if (minVal >= 1000 && maxVal >= 1000)
            return { min: minVal, max: maxVal, vesselType: null };
    }
    ...
    if (labelPrefixSingle) {
        ...
        if (val >= 1000)
            return { min: val, max: val, vesselType: null };
    }
...
function resolvePort(text) {
    const upper = text.trim().toUpperCase();
    return PORT_ABBREVS[upper] ?? text.trim();
}
function cleanBrokerLocation(text) {
    if (!text)
        return null;
    const cleaned = text
...
function extractRoutePorts(text) {
    const normalized = compactForFallback(text);
    const slashRoute = normalized.match(/\b\d+\s*(?:SP|P)?\s+([A-Z][A-Z0-9 .'-]+?)\s*\/\s*\d+\s*(?:SP|P)?\s+([A-Z][A-Z0-9 .'-]+?)(?=\s+(?:\d{4,6}\s*MT|LAYCAN|LC|CARGO|COMM|ADCOM)|$)/i) ||
...
function cleanCargoName(raw) {
    ...
    const cleaned = raw
        ...
        .replace(/\s+/g, " ")
        .trim();
    if (/^\d/.test(cleaned) || /\b\d+\s*(?:SP|P)\b/i.test(cleaned))
        return null;
    if (!cleaned || cleaned.length < 3 || !isValidCargo(cleaned))
        return null;
    return normalizeCargo(cleaned);
}
function extractCargoName(segment) {
    // Explicit CARGO / COMMODITY label — COLON REQUIRED to avoid "cargo inquiry" / "cargo position" false matches
    const explicit = segment.match(/(?:CARGO|COMMODITY|COMMODIT)\s*:\s*([^\n\-–*]+)/i) ||
    ...
    const inlineCargo = compact.match(/\b\d{1,3}[,\s]\d{3}\s*(?:MT|MTS)\s+(?:\d{1,2}\s*%|MOLOO|MOLCO|CHOPT|TTL|TOTAL)?\s*([A-Z][A-Z+]{2,30})\s+(?:IN\s+BULK|LAYCAN|LC\b|CARGO|LOAD|DISCH)/i) ||
...
function parseDuration(text) {
    const wogMatch = text.match(/ABT\s+(\d+)\s+(?:DAYS?|DYS?)\s+WOG/i);
    if (wogMatch) {
        return `${wogMatch[1]} days`;
    }
    const mosMatch = text.match(/UPTO\s+(\d+)\s*\/\s*(\d+)\s+MOS?/i);
    if (mosMatch) {
    ...
    if (rangeMatch) {
        ...
        const unitStr = u.startsWith("MONTH") || u.startsWith("MO")
            ? "months"
...
function parseCompactRates(text) {
...
function extractCommonTechnicalFields(segment) {
    ...
    if (grainMatch) {
        ...
        if (!isNaN(v) && v > 1000)
            fields.grain_capacity = v.toString();
    }
    // Commission
    ...
    if (builtMatch) {
        ...
        if (yr >= 1970 && yr <= new Date().getFullYear() + 1)
            fields.built_year = rawYr;
    }
    // Flag
    const flagMatch = segment.match(PATTERNS.flag);
    if (flagMatch)
        fields.flag = flagMatch[1].trim();
    ...
    if (!fields.load_rate || !fields.discharge_rate) {
        ...
        if (!fields.load_rate && loadRate)
            fields.load_rate = loadRate;
        if (!fields.discharge_rate && dischargeRate)
            fields.discharge_rate = dischargeRate;
    }
    return fields;
}
...
function startsNewBlock(line) {
    ...
    if (/^(?:M\/[TV]|M[TV])(?:[\/\.]?)\s+[A-Z]/.test(t))
        return true;
    if (/^(?:M\/[TV]|M[TV])(?:[\/\.]?)\s+[A-Z].*\bOPEN\b/i.test(line))
        return true;
    // VESSEL: or TONNAGE: — but NOT VESSEL TYPE:, VESSEL POSITION:, etc. (those are field labels not entry headers)
    if (/^VESSEL\s*(?:NAME|DETAILS|OFFER|DESCRIPTION)?\s*:/.test(t))
        return true;
    if (/^TONNAGE\s*(?:POSITION|DETAILS|OFFER)?[:\s]/.test(t))
        return true;
    // Forwarded email separator
    if (/^[-]{3,}\s*(?:Forwarded|Original)\s+(?:message|Message|Msg)/i.test(line))
        return true;
    return false;
}
function splitByAcCt(text) {
    const lines = text.split("\n");
    const blocks = [];
    let current = [];
    let blockCount = 0;
    ...
    if (current.join("").trim().length > 20)
        blocks.push(current.join("\n").trim());
...
function splitByBullets(text) {
    ...
    if (current.join("").trim().length > 30)
        blocks.push(current.join("\n").trim());
    return blocks.length > 1 ? blocks : [];
}
function splitForwardedChains(text) {
...
function segmentEmail(emailText) {
    ...
    const filtered = deduped.filter(segment => !isTechnicalProfileSegment(segment));
    return filtered.length > 0 ? filtered : (deduped.length > 0 ? deduped : [normalizedText]);
}
function isTechnicalProfileSegment(segment) {
    const upper = segment.toUpperCase();
    // Keep any block that clearly contains opening, trading, or cargo information.
    if (/(?:\bOPEN\b|\bO\/A\b|\bSPOT\b|\bPROMPT\b|\bTRY\s+ANY\b|\bTCT\b|\bLAYCAN\b|\bCARGO\b|\bLOAD(?:ING)?\s*PORT\b|\bDISCHARGE\s*PORT\b|\bDEL\b|\bREDEL\b|\bACCOUNT\b|\bCHARTERER\b|\bPERIOD\b|\bHIRE\b|\bFREIGHT\b|\bPOL\b|\bPOD\b|\bQUANTITY\b)/i.test(segment)) {
        return false;
    }
    ...
    const technicalCount = technicalSignals.reduce((count, re) => count + (re.test(upper) ? 1 : 0), 0);
    if (technicalCount < 2)
        return false;
    const hasTradingInfo = /\b(?:ACCOUNT|CHARTERER|RATE|FREIGHT|QUANTITY|LOAD(?:ING)?\s*PORT|DISCHARGE\s*PORT|POL|POD|DELIVERY|REDELIVERY|LAYCAN|OPEN|CARGO|TRY|PERIOD|HIRE|TCT|SPOT|PROMPT|BASIS)\b/i.test(segment);
    const hasVesselHeading = /^(?:M\/V|MV|VESSEL)\b/i.test(segment.trim()) || /\bM\/?V\.?\s+[A-Z]/i.test(segment);
    const hasTechnicalSummaryBlocks = /\bLADEN\b|\bBALLAST\b|\bECO\s+SPEED\b|\bWRKNG\s+CRANES?\b|\bSPEED\/CONSUMPTION\b|\bGRAIN\s+CAPACITY\b|\bGRT\s*\/\s*NRT\b/.test(upper);
    ...
    const hasCargoPortOrCharter = /\b(?:ACCOUNT|CHARTERER|RATE|FREIGHT|QUANTITY|LOAD(?:ING)?\s*PORT|DISCHARGE\s*PORT|POL|POD|DELIVERY|REDELIVERY|LAYCAN|OPEN(?:\s+AT)?|O\/A|CARGO(?!\s+HOLDS?)|TRY|PERIOD|HIRE|TCT|SPOT|PROMPT|BASIS)\b/i.test(segment);
    ...
    const hasVesselHeader = /^(?:M\/V|MV|VESSEL)\b/i.test(segment.trim()) || /\bM\/?V\.?\s+[A-Z]/i.test(segment);
    ...
    if (hasTechnicalProfileSummary && !hasCargoPortOrCharter)
        return true;
    if (hasTechnicalFuelLines && !hasCargoPortOrCharter && technicalCount >= 2)
        return true;
    if (hasVesselHeader && hasTechnicalShipSpecs && !hasCargoPortOrCharter && !/\bOPEN\b|\bO\/A\b|\bOPEN\s+AT\b|\bSPOT\b|\bPROMPT\b/i.test(upper))
        return true;
    if (hasTechnicalSummaryBlocks && hasTechnicalShipSpecs && !hasCargoPortOrCharter)
        return true;
    if (hasNonActionableHeaders)
        return true;
    return false;
}
// ─── Type Detection ───────────────────────────────────────────────────────────
...
function detectSegmentType(segment) {
    if (isTechnicalProfileSegment(segment))
        return null;
    const upper = segment.toUpperCase();
    ...
    TONNAGE_KW.forEach(k => {
        if (normalized.includes(k))
            tonnageScore++;
    });
    const hasTCSignals = /(?:DELY?|DEL|DELIVERY)\s*[:\s*]+|(?:REDELY?|REDEL|RE-DELY?)\s*[:\s*]+|TCT\b|TIME\s*CHARTER|1\s*TCT|\bTRIP\b|\bPERIOD\b|\bDURATION\b|\bHIRE\b/.test(upper);
    ...
    const hasVCSignals = /(?:LP|LOADING\s*PORT?|POL|POD|DISCHARGE\s*PORT)\s*[:\s]+|VOYAGE\s*CHARTER|LOAD\s*RATE|DISRATE|DISCHARGING\s*RATE|\b\d{4,6}\s*MT\b|\bMTS\b|\bIN\s+BULK\b|\b\d+(?:\.\d+)?K\b|\b\d+(?:\.\d+)?\s*-\s*\d+(?:\.\d+)?K\b|\bSF\s*\d/i.test(upper)
        ||
    ...
    const hasMvHeading = /\bM\s*\/?V\.?\b/i.test(segment) || /\bMV\.?\s+[A-Z]/i.test(segment);
    const hasTonnageSignals = (hasMvHeading && /\b(?:OPEN|O\/A|OPEN\s+AT|DWT|IMO|BUILT|BLT|BULK\s*CARRIER|WILL\s*OPEN|OPEN\s*FOR)\b/.test(upper)) ||
    ...
    if (hasTonnageSignals)
        return "Tonnage";
    if (hasTCSignals && !hasVCSignals)
        return "TC";
    if (hasTCSignals && hasVCSignals)
        return "TC"; // prefer TC when both, route ports handle the rest
    if (hasVCSignals)
        return "VC";
    // Fallback heuristics
    if (/CARGO\s*[:\s]/i.test(segment) && /QUANTITY\s*[:\s]/i.test(segment))
        return "VC";
    if (/CARGO\s*[:\s]/i.test(segment) && /DELY?\s*[:\s*]+/i.test(segment))
        return "TC";
    if (parseQuantity(segment).min ||
...
// ─── Entry Extractors ─────────────────────────────────────────────────────────
function extractVCEntry(segment, signature) {
    ...
    if (restrMatches)
        restrictions.push(...restrMatches.map(r => r.trim()));
    ...
    if (end)
        conf += 0.04;
    if (technical.commission)
        conf += 0.03;
    if (technical.load_rate)
        conf += 0.03;
    if (resolveRegion(segment))
        conf += 0.03;
    const fields = {
        email_type: "VC",
        cargo_name: cargo,
...
function extractTCEntry(segment, signature) {
    const accountMatch = segment.match(PATTERNS.account);
    const cargoMatch = segment.match(/(?:CARGO|COMMODITY)\s*:\s*([^\n*]+)/i) ||
        segment.match(/\d\s+TCT\s+(?:WITH\s+)?([A-Z][A-Z\s\/OR]+?)\s+IN\s+BLK/i) ||
        segment.match(/TCT\s+(?:WITH\s+)?([A-Z][A-Z\s\/OR]+?)\s+IN\s+BLK/i);
    ...
    if (restrMatches)
        restrictions.push(...restrMatches.map(r => r.trim()).slice(0, 5));
    const technical = extractCommonTechnicalFields(segment);
    // Confidence scoring
    let conf = 0.40;
    if (accountMatch)
...
function extractTonnageEntry(segment, signature) {
    const mvMatch = segment.match(PATTERNS.mvName) ||
    ...
    // Vessel type
    const vesselType = /TANKER/i.test(segment) ? "Crude Oil Tanker" :
    ...
    const openMatch = splitOpenMatch ||
    ...
    if (restrMatches)
        restrictions.push(...restrMatches.slice(0, 3));
    const technical = extractCommonTechnicalFields(segment);
    // Confidence scoring
    let conf = 0.40;
...
function buildRestrictions(field) {
    if (!field)
        return [];
    return field.split(/;\s*/).filter(p => p.trim().length > 3).map(p => p.trim());
}
function toTonnageEntry(entry) {
    ...
    else if (f.min_size && f.min_size >= 1000)
        dwt = Math.round(f.min_size).toString();
    return {
        email_type: "Tonnage",
...
function toEnterpriseEntry(entry) {
    if (entry.entryType === "Tonnage")
        return toTonnageEntry(entry);
    if (entry.entryType === "TC")
        return toTCEntry(entry);
    return toVCEntry(entry);
}
// ─── Validation ───────────────────────────────────────────────────────────────
function validateEnterpriseEntry(entry) {
    const out = { ...entry };
    if (out.email_type === "Tonnage") {
        const t = out;
        if (!isValidPort(t.open_port))
            t.open_port = "";
        if (t.dwt) {
            const n = parseInt(t.dwt, 10);
            if (isNaN(n) || n < 1000)
                t.dwt = "";
        }
        ...
        const t = out;
        if (!isValidPort(t.del_port))
            t.del_port = "";
        if (!isValidPort(t.redel_port))
            t.redel_port = "";
        if (!isValidCargo(t.cargo)) {
            t.cargo = "";
            t.cargo_type = "";
        }
        ...
        const t = out;
        if (!isValidPort(t.load_port))
            t.load_port = "";
        if (!isValidPort(t.discharge_port))
            t.discharge_port = "";
        if (!isValidCargo(t.cargo)) {
            t.cargo = "";
            t.cargo_type = "";
...
function mergeML(fields, ml, segment, segmentType) {
    if (!ml) {
        return fields;
    }
    const upper = segment.toUpperCase();
    const mlCargo = typeof ml.CARGO === "string" ? ml.CARGO.trim() : "";
    const mlVessel = typeof ml.VESSEL === "string" ? ml.VESSEL.trim() : "";
    const mlPort = typeof ml.PORT === "string" ? ml.PORT.trim() : "";
    const mlEta = typeof ml.ETA === "string" ? ml.ETA.trim() : "";
    const phraseInSegment = (value) => typeof value === "string" && value.trim().length > 0 && upper.includes(value.trim().toUpperCase());
    ...
    if (!fields.laycan_start_date &&
        ml.LAYCAN &&
        /\d/.test(ml.LAYCAN) &&
        hasLaycanHint) {
        fields.laycan_start_date = ml.LAYCAN;
    }
    if (!fields.laycan_end_date &&
        ml.LAYCAN &&
        /\d/.test(ml.LAYCAN) &&
        hasLaycanHint) {
        fields.laycan_end_date = ml.LAYCAN;
    }
    if (fields.email_type === "VC" &&
        !fields.load_port &&
        ml.LOAD_PORT &&
        ml.LOAD_PORT !== "/" &&
    ...
    if (!fields.redel_port &&
        fields.email_type === "VC" &&
        ml.DISCHARGE_PORT &&
        (phraseInSegment(ml.DISCHARGE_PORT) || /\bDISCHARGE\s*PORT\b|\bPOD\b/i.test(segment))) {
        fields.redel_port = ml.DISCHARGE_PORT;
    }
    if (!fields.email_id && ml.CONTACT && /@/.test(ml.CONTACT))
        fields.email_id = ml.CONTACT;
    if (!fields.pic && ml.CONTACT && !/@/.test(ml.CONTACT))
        fields.pic = ml.CONTACT;
    if (!fields.tonnage_name &&
        mlVessel &&
        segmentType === "Tonnage" &&
        phraseInSegment(mlVessel)) {
        fields.tonnage_name = mlVessel;
    }
    if (!fields.tonnage_type &&
        ml.VESSEL_TYPE &&
        segmentType === "Tonnage" &&
    ...
    if (!fields.open_date &&
        mlEta &&
        segmentType === "Tonnage" &&
        phraseInSegment(mlEta)) {
        fields.open_date = mlEta;
    }
    if (!fields.dwt &&
        ml.DWT &&
        segmentType === "Tonnage" &&
...
function extractMaritimeEmail(emailText) {
    const start = Date.now();
    console.log("INSIDE EXTRACT MARITIME EMAIL");
    const preprocessed = sanitizeEmail(preprocessEmail(emailText));
    console.log("STEP 1");
    console.log("ABOUT TO CALL ML");
    let mlResult = {};
    try {
        mlResult =
            (0, mlConnector_js_1.runMLModel)(preprocessed);
    ...
    console.log("STEP 3");
    const pipeline = template.name ? "template" : "rule-based";
    for (const segment of segments) {
        ...
        entry.fields =
            mergeML(entry.fields, mlResult, segment, segType);
        console.log("\n========== AFTER MERGE ==========");
        console.log(JSON.stringify(entry.fields, null, 2));
        if (template.boost > 0) {
    ...
    if (entries.length === 0) {
        ...
        if (fallbackType) {
            let entry;
            if (fallbackType === "VC")
                entry = extractVCEntry(preprocessed, signature);
            else if (fallbackType === "TC")
                entry = extractTCEntry(preprocessed, signature);
            else
                entry = extractTonnageEntry(preprocessed, signature);
            console.log("\n========== FALLBACK PURE REGEX ==========");
            console.log(JSON.stringify(entry.fields, null, 2));
            entry.fields =
                mergeML(entry.fields, mlResult, preprocessed, fallbackType);
...
function extractToEnterpriseJSON(emailText) {
